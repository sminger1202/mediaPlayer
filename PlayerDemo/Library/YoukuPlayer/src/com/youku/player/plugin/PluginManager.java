...
public class PluginManager {
	...
	public PluginManager(FragmentActivity youkuBasePlayerActivity) {
	...
	public void onCompletionListener() {
		Logger.d(LogTag.TAG_PLAYER, "onCompletionListener");
		for (PluginOverlay pluginOverlay : pluginOverlays) {
	...
	public void onLoaded() {
		Logger.d(LogTag.TAG_PLAYER, "通知视频加载成功 onLoaded");
		for (PluginOverlay pluginOverlay : pluginOverlays) {
	...
	public void onLoading() {
		Logger.d(LogTag.TAG_PLAYER, "通知视频加载 onLoading");
		for (PluginOverlay pluginOverlay : pluginOverlays) {
	...
	public void onSubtitlePrepared() {
		Logger.d(SubtitleManager.TAG, "onSubtitlePrepared()");
		for (PluginOverlay pluginOverlay : pluginOverlays) {
			pluginOverlay.onSubtitlePrepared();
		}
	}
	...
	public void onBufferPercentUpdate(int percent) {
		for (PluginOverlay pluginOverlay : pluginOverlays) {
			pluginOverlay.onBufferPercentUpdate(percent);
		}
	}
	
		public void needDownloadDRMSo(String soName) {
		for (PluginOverlay pluginOverlay : pluginOverlays) {
			pluginOverlay.needDownloadDRMSo(soName);
		}
	}
	public void set3GTips() {
		for (PluginOverlay pluginOverlay : pluginOverlays) {
			pluginOverlay.set3GTips();
		}
	}
	...
	public void onADplaying() {
		for (PluginOverlay pluginOverlay : pluginOverlays) {
			pluginOverlay.onADplaying();
		}
	}

	public void viewPositionChange(View view, float offset) {
		for (PluginOverlay pluginOverlay : pluginOverlays) {
			pluginOverlay.viewPositionChange(view, offset);
		}
	}
}