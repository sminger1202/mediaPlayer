...
public class AcceleraterServiceManager {
	...
	public static boolean isAccSupported(Context context) {
		...
		Logger.d(TAG, "android.os.Build.MODEL = " + android.os.Build.MODEL + ", android.os.Build.BRAND = " + android.os.Build.BRAND);
		
		String brand = android.os.Build.BRAND + "_" + android.os.Build.MODEL;
		
		Logger.d(TAG, "brand = " + brand + ", sBlackList = " + sBlackList + ", iscontain = " + sBlackList.contains(brand));
		
		if (!android.os.Build.CPU_ABI.toLowerCase().contains("armeabi") || sBlackList.contains(brand)) {
			Logger.e(TAG, "11-体系结构为:" + android.os.Build.CPU_ABI.toLowerCase() + ";brand=" + brand);
			sFailReason = "11-体系结构为:" + android.os.Build.CPU_ABI.toLowerCase() + ";brand=" + brand;
			return false;
		}
	...
	public static boolean getDownloadSwitch() {
		...
		return AccInitData.getDownloadSwitch(context);
	}
	
	public static boolean getPlaySwitch() {
		Context context = com.baseproject.utils.Profile.mContext;
		
		return AccInitData.getPlaySwitch(context);
	}
	...
	public static boolean canDownloadWithP2P() {
		...
		if (isACCEnable() && AccInitData.getDownloadSwitch(context)) {
	...
	public static boolean canPlayWithP2P() {
		...
		if (isACCEnable() && AccInitData.getPlaySwitch(context)) {
	...
	public static void postEventFromNative(int what, int arg1, int arg2, Object obj) {
		Logger.d(TAG, "postEventFromNative : what = " + what);
	}
