...
public class ImageResizer extends ImageWorker {
	 ...
	 * @return
	 */
	private Bitmap processBitmap(int resId, ImageView mImageView) {
//		Logger.d(TAG, "processBitmap - " + resId);
		if (!isNeedCropHeight) {
			if (mImageView != null && mImageView.getWidth() > 0 && mImageView.getHeight() > 0) {
				return decodeSampledBitmapFromResource(mContext.getResources(),
						resId, mImageView.getWidth(), mImageView.getHeight());
			} else {
				return decodeSampledBitmapFromResource(mContext.getResources(),
			...
			Bitmap b = null;
			if (mImageView != null && mImageView.getWidth() > 0 && mImageView.getHeight() > 0) {
				b = decodeSampledBitmapFromResource(mContext.getResources(),
						resId, mImageView.getWidth(), mImageView.getHeight());
			} else {
				b = decodeSampledBitmapFromResource(mContext.getResources(),
						resId, mImageWidth, mImageHeight);
			...
			int h = b.getHeight();
			return Bitmap
					.createBitmap(b, 0, 0, w, h - mCropHeight, null, false);
		}
	}

	@Override
	protected Bitmap processBitmap(Object data, ImageView mImageView) {
		return processBitmap(Integer.parseInt(String.valueOf(data)), mImageView);
	}
	...
	public synchronized Bitmap decodeSampledBitmapFromFile(String filename,
		...
		try {
			...
			System.gc();
//			Logger.e(Youku.TAG_GLOBAL, "ImageResizer#processBitmap()", e);
		}
		return b;
	}
...
//		 System.out.println("inSampleSize:" + inSampleSize + ",width:" + width
//		 + ",height:" + height + ",mImageWidth:" + mImageWidth +
