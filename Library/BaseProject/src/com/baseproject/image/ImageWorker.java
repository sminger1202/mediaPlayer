...
public abstract class ImageWorker {
	...
	public static final ImageWorker getInstance(FragmentActivity context) {
		if (null == imageWorker) {
			final int height = Device.ht;
			final int width = Device.wt;
			final int shortest = height > width ? width : height;
			imageWorker = new ImageFetcher(context, shortest);
			imageWorker.setImageCache(ImageCache.findOrCreateCache(context,
	 ...
	 */
	protected abstract Bitmap processBitmap(Object data, ImageView mImageView);
	...
	private class BitmapWorkerTask extends ImageAsyncTask<Object, Void, Bitmap> {
		...
		protected Bitmap doInBackground(Object... params) {
			...
			if (bitmap == null && !isCancelled() && getAttachedImageView() != null && !mExitTasksEarly) {
				...
				bitmap = processBitmap(params[0], mImageView);
			}
			...
			if (null != mImageCache) {
				if (null != bitmap) {
					...
					if(!TextUtils.isEmpty(dataString)) {
						mImageCache.getDiskCache().removeFile(Utils.urlToFileName(dataString));
					}
				}
			}
