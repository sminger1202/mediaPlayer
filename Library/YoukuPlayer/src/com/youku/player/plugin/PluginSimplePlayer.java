...
public class PluginSimplePlayer extends PluginOverlay implements DetailMessage {
	...
	public PluginSimplePlayer(Activity mActivity,
	...
	public void showLoading() {
		...
		if (mMediaPlayerDelegate.isADShowing) {
			Logger.d(TAG, "mMediaPlayerDelegate.isADShowing()");
			return;
		}
		if (null != seekLoadingContainerView) {
    ...
    private void startPlayByAdButton() {
        if (mMediaPlayerDelegate == null || mActivity == null)
            return;
        if (mMediaPlayerDelegate.isPause && mMediaPlayerDelegate.isAdvShowFinished()) {
            mMediaPlayerDelegate.isPause = false;
        } else {
            mMediaPlayerDelegate.start();
            if (mMediaPlayerDelegate.videoInfo != null
                    && !mMediaPlayerDelegate.videoInfo.isAdvEmpty()) {
                mMediaPlayerDelegate.seekToPausedADShowing(mMediaPlayerDelegate.getAdPausedPosition());
            }
        }
    }
	...
	public void OnPreparedListener() {
		Logger.d(TAG, " OnPreparedListener()");
		seekcount = 0;
		if (null != retryView)
			retryView.setVisibility(View.GONE);
	}
	...
	protected void startCache() {
		if (MediaPlayerDelegate.mIUserInfo == null
				|| !MediaPlayerDelegate.mIUserInfo.isLogin()) {
			// Util.showToast("请先登录");
			return;
		}
	}
